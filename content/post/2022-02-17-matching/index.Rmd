...
But wait, we still have a way out in situations like that. We can leverage the **conditional independence assumption**.
...
For our matching estimators this means that as more variables are considered in the covariate matrix X, the likelihood of finding exact matches[^9] for all units falls quickly. So, it may be almost impossible to meet the common support assumption when using a lot of variables in the matching process, unless we have also *a lot* of data.
...
As we saw, if we have to control for several features it's unlikely we'll find exact matches for all the treated units, let alone for the whole sample. Because of this, it's common to, instead of looking for perfect matches, use a proximity or distance metric that tell us how different or similar any two pair of units are, and then match each unit with the most similar unit with opposite treatment assignment.

But before jumping into match each unit with its closest(s) neighbour(s), there are a couple of previous steps we should carry out.

### Scaling and choosing a distance metric

The first step is **scaling the covariates X**[^11]. This means dividing each feature by the corresponding standard deviation. The result is all covariates having a standard deviation equal to 1. If we don't do this, some features will have more importance in the final proximity calculation just because of their measurement units.

[^11]: Again, if you come from the ML world this is evident and straightforward, but I'm going to explain it anyway for people with different backgrounds.

For example, if one of the covariates is `age` and its measured in years, and another is `daily_food_intake` and its measured in calories, then the later will have much more influence on the distance, just because it's measured in a scale with higher values and more variation. What's more problematic, we could change the influence of each covariate just by transforming it to another equivalent scale, e.g. increase the weight of age by measuring it in months or days. Obviously, all of this is undesirable, so that's why you must scale your covariates.

Then the second step is to choose a distance or proximity metric.

The most common distance metric is the euclidean distance, which pretty much sums up to representing each observation as a point in a hyperplane with M dimensions (with M as the number of features) and then computing the length of a straight line between those two points. If you have calculated a distance between two points in a Cartesian plane, it probably was euclidian distance.
...
The top-left image represents the raw, original feature-space of two correlated variables, and the bottom-left represents what the space we end up with after dividing out the covariance matrix. Then, an euclidian distance is calculated in this transformed space. That is the Mahalanobis distance.

Note that only doing the scaling can be seen as "stretching" the original space to achieve equal variance among all the dimensions, but dividing by the covariance implies also "rotating" it in order to eliminate the correlation[^12].

[^12]: If you're familiar with Principal Component Analysis, it may be helpful to see Mahalanobis distance as just euclidian distance in a PCA-rotated and standarised space, that is, a feature-space in which you applied PCA, but kept all the resulting uncorrelated features (i.e. no dimensionality reduction) and then scaled them to have the same variance.

Okey, that was the "intuitive" explanation. Now let's express this metric in code:

```{r}
# `x_i` and `x_j` are two numeric vectors that represent two observations in the
# data. Both vectors have the same length M, where M is the number of covariates
# or features

# cov is the variance-covariance matrix of variables X, a MxM matrix
...
So the proximity metric thing wasn't actually a choice because the Mahalanobis distance is always the best option. Good. Also, we don't have to decide on the bin sizes for continuous variables anymore (as we had to do with exact matching). However, approximate matching still require us to take some other decisions.

-   **How many matches we'll use for each unit?** After calculating distances, we will have a sorted ranking of potential matches for each unit. So, how many units from that list we'll use to impute the counterfactual? Just one (the closest)? Two? Or more?[^13].

-   **Calliper** (also known as bandwidth): how far can two units be before we stop considering them as potential matches? Note that if we choose a very narrow calliper, we could end up with a lot of unmatched units.
...
[^13]: If you have worked with the K-nearest neighbours algorithm, this is equivalent to choosing the K parameter.
...
By doing this, we're ensuring that the matched samples are as similar as possible to the original units in their covariates values (i.e. that they have minimum bias). This, in turn, maximises our chances of closing the nasty backdoors in our DAG.
...
However, this setting also maximises variance: if we use just one match to estimate each counterfactual, then that estimate is going to be very noisy. And the problem becomes worse if we allow each unit in the donor pool to be matched to several original units. This is bad because we become susceptible to sample variance or random variation in the observed data. The *expected value* of the estimator will be close to the true ATT or ATE (little bias) but the *realisation* we observe given our current sample may fall far off from that.

To reduce variance, we may consider using more matches for each unit, widening the calliper, and doing matching *without* replacement, but all this would increase bias.

![](images/choose_variance_bias.jpg)

So what should we do? For the number of matches, [*The Effect* suggest to use more matches when our dataset is big](https://theeffectbook.net/ch-Matching.html#:~:text=So%20the%20choice,Not%20worth%20it.) because then we can reduce variance at the expense of just a small increase in bias. But if the data is small, the price of such variance reduction is much higher, since the second-best or third-best matches could be far off the original unit in their covariate values.

Regarding replacement and calliper/bandwidth, I guess a similar principle comes into play. For bigger datasets we could "afford" choices that reduce variance such as not using replacement and increase the bandwidth, but for small data it may be too much to ask[^14].

[^14]: *The Effect* also points out that there is extensive literature on how to choose the optimal bandwidth/calliper when doing approximate matching, so you may want to look at that.

### Distance Matching Estimator ✨as code✨

Now let's implement a distance matching estimator as an R function. For this example, I'm going to pick an estimator with replacement, without calliper restriction and with just one match per unit (the best/closest). This is not necessarily the most appropriate estimator for *your* data, but it's the easier to code and understand[^15].

[^15]: And anyway, the purpose of these code examples is just to explain what these estimators do. If you really want to use matching estimators in your analysis, you should use an implementation from an R package such as `MatchIt`. I'm going to cover this in Part 2 of this blog post.
...
distance_matching_estimator_att <- function(df,
  ...
  # Copying the covariates in a separate DF to calcule the variance matrix
  covariates <- df %>% 
    select({{ covariates }})
  ...
  # Getting the outome values of control matches
...
But after using distance matching to adjust for the confounders, we found a positive effect: the `medication` seems to reduce the `recovery` period in 6.74 days on average (for the treated group).
...
Okay, this section has been pretty long, but that's because talking about the distance matching estimator entails necessarily talking about all those topics. And there is still one more topic left! It's the **bias correction method** that is often used when doing approximate matching.

Since it's almost impossible for the distance between two units to be zero, we can take for granted that there will be some bias due to differences in covariates between the original and the matched units. Fortunately, [Abadie and Imbens (2011)](https://economics.mit.edu/files/11861) introduced a bias correction method, which leverages the fact that discrepancies in coviariates are *observed*.

You can read the original paper or [the reference](https://mixtape.scunning.com/matching-and-subclassification.html?panelset3=r-code4#bias-correction) [books](https://matheusfacure.github.io/python-causality-handbook/10-Matching.html#matching-bias) if you want to go into more detail, but, for the ATT, it basically mounts up to each unit contributing this expression to the total amount of bias in the estimator[^16]:

[^16]: I REALLY wanted this blog post to have zero math equations and express literally everything as code, but I realised that doesn't make sense for equations that represent "true" parameters or functions, since they can't be calculated. On the other hand, code can represent estimators very well, because they're actually algorithms that are meant to be applied on real data.
...
How to estimate the bias? A way of doing it is fitting a linear regression with the covariates as predictors and the outcome and response, *but using only data from the untreated units*. Then we'll feed $X_i$ and $X_{j(i)}$ to the fitted regression in order to obtain corresponding predicted values. The difference between those predicted values is our estimate of the bias for the untreated matched sample.
...
distance_matching_att_w_bias_adj <- function(df,
  ...
  matched_controls_values <- 
    ...
    # and the coviariates in the control matched sample
    mutate(u0_j = predict(u0)) %>% 
    select(outcome_match := {{ outcome }},
           u0_j)
...
![](C:/Users/franc/AppData/Local/RStudio/tmp/paste-820A6CF1.png){width="800"}
