...
The first one is **scaling the covariates X**[^11]. This means basically subtracting the corresponding mean to each feature, and then dividing by the corresponding standard deviation. The result of this is all covariates having a mean equal to 0 and a standard deviation equal to 1. If we don't do this, some features will have more importance in the final proximity calculation just because of their scale.
...
For example, if one of the covariates is `age` and its measured in years, and another is `daily_food_intake` and its measured in calories, then the later will have much more influence on the distance, just because it's measured in a scale with higher values and more variation. What's more ridiculous, we could change the influence of each covariate just by transforming it to another equivalent scale, e.g. increase the weight of age by measuring it in months or days. Obviously, all of this is undesirable, so that's why you must scale your covariates.

Then the second step is to choose a distance or proximity metric.

The most common distance metric is the euclidean distance, which pretty much sums up to representing each observation as a point in a hyperplane with M dimensions (with M as the number of features) and then computing the length of a straight line between those two points.

```{r}
scaled_euclidian_distance <- function(x_i, x_j, sd) {
  x_i <- x_i/sd
  x_j <- x_j/sd
...
```{r}
# x and y represent two observations with M features
# i.e. vectors of length M.
# Also this function assumes that features are already standarised
euclidian_distance <- function(x, y, sd) {

  map2_dbl(x, y, ~abs(.x - .y)) %>% 
    map2_dbl(.y = sd, ~.x/.y) %>%
    map_dbl(.f = ~.^2) %>% 
    reduce(sum) %>% 
    sqrt()
...
However, we can do better. According to The Effect, the best distance metric to use with approximate matching is the Mahalanobis distance. This metric is closely related to Principal Component Analysis, in the sense that it projects the vectors (observations) into a space where there is no correlation between the features

Why would we want to do that? Because we could have two or more highly correlated variables, for example, gender and

```{r}
mahalanobis_distance <- function(x_i, x_j, cov){
  inv_covar <- matlib::inv(cov)
  
  matrix_mult_output <-
    (t(as.matrix(x_i - x_j)) %*% inv_covar %*% as.matrix(x_i - x_j))
  
  sqrt(matrix_mult_output)
}
```

Why this distance metric is better?
...
sd_cars <- matrixStats::colSds(as.matrix(cars))


## Add euclidian distance
cars %>% 
  rowwise() %>% 
  mutate(euclidian_distance = euclidian_distance(c(.$displ, .$hwy),
                                                          y = c(4.7, 17),
                                                          sd = sd_cars))
```
...
-   Other things to decide: caliper, number of neighbours, replacement or not.

(explain that there a lot of different parameters to decide when we do matching... but here we're going to use a specifical... talk about the "questions" that The Effect says we have to ask)

## Bias correction in distance matching
