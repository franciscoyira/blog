...
title: 'Building an Airflow Pipeline That Talks
  to AWS — Data Pipelines in the Cloud (III)'
...
Welcome back everyone to the third part of this series on Data Pipelines in the Cloud! In the [first part](post/data-pipelines-cloud-intro-airflow-docker/), I introduced Apache Airflow, a powerful tool for orchestrating data pipelines. In the [second part](post/data-pipelines-2-cloud-aws-command-line-s3-rds/), we learned how to interact with Amazon Web Services (AWS) using the command line. And today, **we will put all the pieces together and build an Airflow pipeline that talks to AWS** 🥳.
...
>
> -   [Part II: Using Amazon Web Services with the Command Line](post/data-pipelines-2-cloud-aws-command-line-s3-rds/)
>
...
-   You have the **AWS Command Line Interface tool installed and configured with an AWS account**. If you need help with that, check [the second part of this series](post/data-pipelines-2-cloud-aws-command-line-s3-rds/). **There you can also find detailed explanations of all the AWS CLI commands and AWS services used in this third part**. I’ll skip explaining these again in this new post to focus on the Airflow DAG instead.
...
-   Performs some meaningful **data transformations** in an automated way (below more details about what these transformations are).
-   **UPSERTS** this transformed data into a **PostgreSQL database**, running on Amazon RDS. *UPSERT* means that data corresponding to *new* records will be appended to the table, whereas data about *existing* records will overwrite or update the previous information. A Primary Key column will be used to determine whether a record is new or not.
...
Our source data consists of a sample of [*pomodoro sessions*](https://en.wikipedia.org/wiki/Pomodoro_Technique), that is, short periods of time in which a person performed **focused work** on some project. The data is at the **Session** level of detail, so the columns contain attributes about each session such as:

-   **Project**: indicates if the session was dedicated to Paid Work ("*Work"*) or study and side-projects ("*Learning"*).

-   **Duration (in minutes)**: a decimal number indicating the duration in minutes of the session. Typically 25 minutes, but may vary.

-   **Start date** and **End date**: timestamps in local time indicating when each session started and when it ended.
...
-   Group the data by `Start Date` and `Project`. Note that there could be multiple or no rows for each day and project.
...
![*UPSERT in a nutshell. Source: [data-nerd.blog](https://data-nerd.blog/2021/07/15/postgresql-how-to-upsert-update-or-insert-into-a-table/).*](images/upsert.png){alt="A flowchart with an oval in the center containing the question ‘Key Exists?’ inside the oval. Two arrows emerge from the oval, labeled ‘Yes’ and ‘No’. The ‘Yes’ arrow points to a rectangle labeled ‘UPDATE’, indicating that if the key exists, it should be updated. The ‘No’ arrow points to another rectangle labeled ‘INSERT’, indicating that if the key does not exist, a new one should be inserted." width="900"}

As you can see, the whole pipeline (except the UPSERT, which which we still need to write) can be expressed as a handful of lines of Python code. Still, it performs a meaningful transformation on the data (it isn't just a `print('Hello world')`, [like the dummy DAG we implemented on the first part](/post/data-pipelines-cloud-intro-airflow-docker/)) and outputs a table in a format that may be suitable for dashboards or reports. The main challenge is to perform these transformations in an automated way, under Airflow orchestration, while also interacting with AWS cloud services like S3 and RDS.

## Getting Things Ready

### Cloning The Repo

To get started, clone this repo that contains all code we'll be working with, and then open it in your favourite IDE: <https://github.com/franciscoyira/aws_airflow_tutorial.git>
...
![](images/vs_code_fresh.png){alt="Visual Studio Code interface with a dark theme. The ‘EXPLORER’ panel displays files and folders for a project named ‘AWS AIRFLOW_TUTORIAL.’ The file structure includes folders like ‘config,’ ‘dags,’ and files such as ‘.gitignore,’ ‘docker-compose.yaml,’ and ‘requirements.txt.’ A terminal at the bottom shows the user prompt in the ‘aws_airflow_tutorial’ directory." width="1000"}
...
[*Explanation of the commands above.*](/post/data-pipelines-2-cloud-aws-command-line-s3-rds/#an-s3-bucket-to-store-the-raw-data)

**🚨 Important: take note of the name of the `$bucket_name`**, as we'll need it later when setting up the DAG.

Now, let's provision an Amazon RDS instance and its corresponding security group, enabling incoming traffic from our own IP:
...
[*Explanation of the commands above.*](/post/data-pipelines-2-cloud-aws-command-line-s3-rds/#provisioning-the-database)

In order to connect to the instance, we need to download the corresponding Certificate Authority (CA) and install the PostgreSQL and the jq tool:
...
[*Explanation of the commands above.*](/post/data-pipelines-2-cloud-aws-command-line-s3-rds/#testing-the-connection)

You should see this on screen `airflow_tutorial =>` if the connection was successful. Once there, you can create the table where our processed data will live by executing the following SQL statement:
...
[*Here* *you can find instructions to obtain these keys from AWS*.](/post/data-pipelines-2-cloud-aws-command-line-s3-rds/#configuring-the-aws-cli-with-an-aws-account)
...
Some commands may take a while, but after they're finished you should be able to access the Airflow Web UI using the URL [localhost:8080](http://localhost:8080/) on your web browser:

![*If you have any problems or questions about the Airflow setup, check [the first part of this series](/post/data-pipelines-cloud-intro-airflow-docker/#installing-airflow-on-the-windows-subsystem-for-linux) .*](images/airflow_login.png){alt="Login interface for Airflow on a web browser. The URL indicates it’s hosted on localhost. The ‘Sign In’ section has fields for entering ‘Username’ and ‘Password’." width="1000"}

Here you can login using `airflow` as username and `airflow` as a password.
...
Note that you can also access this code through the Airflow Web UI, by clicking on `transform_and_upsert` from the "DAGs" tab, and then selecting the "*Code*" tab. This is because Airflow automatically monitors the `dags/` folder and parses its contents as proper DAGs that can be managed and monitored on the Web UI. The *Code* tab is super useful to check if Airflow has parsed already any changes you've made to your DAG files.
...
*(Note: all of this code is meant be run by Airflow itself when executing the DAG. It is not designed to run it interactively on your Terminal.)*
...
-   `boto3` is the AWS Software Development Kit (SDK) for Python, which allows Python developers to write software that makes use of services like Amazon S3 and RDS.
-   `s3fs` is a Pythonic file interface to Amazon S3. It builds on top of `boto3` to provide typical file-system style operations like `cp`, `mv`, `ls`, along with put/get of local files to/from S3.

Now, we put these packages to use by creating a `boto3.Sesion()`, which allows us to interact with AWS services and instantiate clients for S3 and RDS.
...
-   `'owner': 'airflow'` defines the user "*airflow*" as the owner of this DAG. Since "*airflow*" is the default username of our Airflow environment, and the one we used to access the Web UI, this gives us the ownership of the DAG.

-   `depends_on_past` controls whether the DAG should be run even if the previous scheduled execution failed. A value of `False` means that the DAG will be triggered anyway under this scenario.
...
-   `'retries': 1` and `'retry_delay': timedelta(minutes=1)` mean that if something fails when running the DAG, Airflow will wait one minute and then will attempt to run it again. If that run also fails, it will give up and the DAG run will be marked as "failed".
...
We use the class constructor `DAG()` to create the `dag` object, passing the `default_args` to it, giving it the name `transform_and_upsert`[^2] and setting up with a *monthly* `schedule` (it should run the first day of each month).
...
-   [`s3.list_objects_v2`](https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/s3/client/list_objects_v2.html) is a function that returns a dictionary with *information about* the objects in our bucket (not the objects themselves). We use the arguments `Prefix` to retrieve just the objects within the `input_data/` directory of the bucket.
...
Most of this function consists of pandas code, which was already explained when we talked about [the input data](/post/data-pipelines-3-cloud-build-airflow-pipeline-aws/#the-input-data), so I'll skip over that and focus just on **the code that interacts with AWS**:
...
Some may wonder why we are using S3 to write temporary files and pass data between tasks when we could use the XCom feature instead. The answer is simple - **the XCom feature is designed for passing small pieces of data, such as variables and parameters. It is not intended for transferring larger datasets or files**. Doing so may result in unreliable behavior of XCom and thus is considered bad practice.
...
![](images/data_pipeline_upsert_focus.png){alt="A flowchart illustrating a data processing workflow using Apache Airflow, AWS S3, and AWS RDS. It begins with ‘Raw Data’ containing project details in an S3 bucket, processed by Apache Airflow. Steps include reading raw CSV from S3, performing pivot transformation, and upserting transformed data to AWS RDS. The final output is stored in an Amazon RDS Database as a ‘Final Table’ with fields for date, work_minutes, and learning_minutes. A note ‘We are here’ indicates the current step in the process." width="1000"}
...
-   `rds_hook.get_sqlalchemy_engine()` is a method that returns a SQLAlchemy engine object, which is required by `to_sql` to connect to the database.
...
-   **`EXCLUDED`:** Represents the row from `tmp_{table}` that would have been inserted if there were no conflict on the `date` column. Therefore, we can use `EXCLUDED.column_name` to update `work_minutes` and `learning_minutes` with the corresponding values from the temporary table.
...
We'll do this by using Airflow's `PythonOperator`, which allows us to **run arbitrary Python functions as part of our DAG**.
...
Now that we have all the tasks defined, we need to specify the order in which they should be executed. This is done by using the `>>` operator to define the dependencies between tasks.
...
In this case, we have two possible paths: - If `read_combines_new_files_from_s3` finds new files in S3, the pipeline will proceed to `pivoting_df` and then to `upsert_task`. - If no new files are found, the pipeline will proceed to `end_task` and the DAG will end.

On the Airflow Web UI, you can view a visual representation of this DAG. Simply go to **DAGs \> transform_and_upsert** and click on the **Graph** button.
...
## Triggering the DAG for Testing
...
To do this, go to **DAGs \> transform_and_upsert** and then click on the **Trigger DAG** button on the upper-right corner of the screen:
...
Then, you can monitor the progress of the DAG by clicking on the **Grid View** button. If everything goes well, after a couple of minutes you should see a something like the highlighted task sequence on the image below *(please ignore all the failed DAG runs I had while debugging this 🤣)*:
...
In the end, it turned out that the problem was that I was passing the wrong Prefix (inputs/ instead of input_data/), which is something that can't be inferred from the log themselves, but at least **they pointed me in the right direction** and helped me to narrow down the problem. That's their role.

## Wrapping Up

And that's it! We've defined a complete data pipeline in Airflow that reads data from an S3 bucket, processes it, and upserts it to a PostgreSQL database. **Congratulations if you made it this far! 🎉**

## Shutting Down AWS Resources

Please remember to [shut down all the AWS resources](/post/data-pipelines-2-cloud-aws-command-line-s3-rds/#wrap-up-shutting-down-resources-to-minimize-costs) used in this tutorial after you're done to save costs and stay within the Free Tier limits. You can do this by running the following commands on your Terminal:
